...
interface RecordFilterProps {
  sections: RecordDTO[];
  onFilter: (filtered: RecordDTO[]) => void;
  allowReorder: boolean;
  setAllowReorder: (allow: boolean) => void;
}


export default function RecordFilter({ sections, onFilter, allowReorder, setAllowReorder }: RecordFilterProps) {
  ...
  return (
    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:gap-6 sm:items-center sm:justify-between">
      ...
      <div className="flex-1">
        <InputGroup>
          <InputGroupInput
            placeholder="Пошук по назві запису"
            value={search}
