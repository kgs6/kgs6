...
export async function POST(req: Request) {
  ...
  if (!user) return Response.json({ error: "Перевірте свої облікові дані" }, { status: 401 });

  const ok = await bcrypt.compare(password, user.password);
  if (!ok) return Response.json({ error: "Перевірте свої облікові дані" }, { status: 401 });
  ...
  res.headers.append("Set-Cookie", cookie("refreshToken", refreshToken, 7 * 24 * 60 * 60));
